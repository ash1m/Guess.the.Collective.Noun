...
const GameCard: React.FC<GameCardProps> = ({
  ...
  return <Card className="w-full max-w-md mx-auto p-6 shadow-xl bg-white rounded-3xl animate-fade-in">
      ...
      <div className="space-y-3">
        {question.options.map((option, index) => <Button key={option} onClick={() => !isAnswered && onAnswerSelect(option)} className={getButtonClass(option)} variant={getButtonVariant(option)} disabled={isAnswered}>
            {option}
          </Button>)}
      </div>
      
      {isAnswered && <div className="mt-6 text-center animate-fade-in">
          {selectedAnswer === correctAnswer ? <div className="text-green-600 font-bold text-xl">
              🎉 Correct! +{attempts === 1 ? 10 : 0} points
            </div> : <div className="text-red-600 font-bold text-lg">
              {attempts >= 3 ? <>❌ The answer was "{correctAnswer}"</> : <>❌ Try again! {3 - attempts} attempts left</>}
            </div>}
        </div>}
    </Card>;
};
